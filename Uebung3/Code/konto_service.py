...
class KontoService(KontoServiceInterface):
    ...
    def konten_auflisten(self) -> List[Dict]:
        return [
            {"id": konto.konto_id, "saldo": konto.saldo}
    ...
    def ueberweisen(self, von_konto_id: int, zu_konto_id: int, betrag: Decimal) -> None:
        ...
        von_konto = next((konto for konto in self._konten if konto.konto_id == von_konto_id))
        zu_konto = next(konto for konto in self._konten if konto.konto_id == zu_konto_id)
